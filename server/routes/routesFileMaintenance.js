...
function deleteData(dataName, req, res) {
   connection
      .query("DELETE FROM tbl" + dataName + " WHERE " + dataName + "ID = ?", [
         req.params.id,
      ])
      .catch((err) => {
         console.log(err);
...
function editData(dataName, input, req, res) {
   connection
      .query(
         "UPDATE tbl" + dataName + " SET name = ? WHERE " + dataName + "ID = ?",
         [input, req.params.id]
      )
      .catch((err) => {
         console.log(err);
...
function addData2(dataName, que, input1, input2, res) {
   connection
      .query("INSERT INTO tbl" + dataName + " " + que, [input1, input2])
      .catch((err) => {
         console.log(err);
      })
      .then((mess) => {
...
router.post("/addBrgy", (req, res) => {
   var { inputAddBrgy, inputAddCity } = req.body;
   addData2(
      "barangay",
      "SET name = ?, cityID = ?",
      inputAddBrgy,
...
router.post("/editBrgy:id", (req, res) => {
   ...
   editData2(
      "barangay",
      "SET name = ?, cityID = ? WHERE barangayID = ?",
      updatedBrgy,
      updatedBrgyCity,
      req,
      res
...
router.post("/editServices:id", (req, res) => {
   ...
   editData2(
      "services",
      "SET name = ?, categoryID = ? WHERE ServicesID = ?",
      updatedServices,
      updatedCategoryServ,
      req,
      res
