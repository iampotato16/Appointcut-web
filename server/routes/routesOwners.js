...
router
   ...
   .get(async (req, res) => {
      ...
      const rowsEmp = await acu.getAllFromWhere(
         "(SELECT e.*, et.Name AS EmployeeType, st.Name AS SalaryType, st.SalaryTypeID, e.salaryTypeValue AS SalaryValue, CONCAT(e.firstName, ' ', e.lastName) AS FullName, e.email AS Email, e.contact AS Contact FROM tblemployee e LEFT JOIN tblemployeetype et ON e.employeeTypeID = et.EmployeeTypeID LEFT JOIN tblsalarytype st ON e.salaryTypeID = st.SalaryTypeID) AS sub",
         "ShopID = " + req.params.shopId
      );

      const rowsServ = await acu.getAllFromWhere(
         "(SELECT ss.*, ss.ShopServicesID AS ID, s.Name AS Service, c.Name AS CategoryName, ss.price AS Price, ss.duration AS Duration FROM tblshopservices ss JOIN tblservices s ON ss.servicesID = s.ServicesID JOIN tblcategory c ON s.CategoryID = c.CategoryID) AS sub",
         "shopID = " + req.params.shopId
      );
      ...
      const rowsEmpSpec = await acu.getAllFromWhere(
         ...
         "EXISTS (SELECT 1 FROM tblshopservices ss2 WHERE ss2.ShopServicesID = sub.shopServicesID AND ss2.shopID = " + req.params.shopId + ")"
      );
...
router.post("/view:ownerId/viewShop:shopId/editEmp:empId", async (req, res) => {
   console.log("barbershop employee edit")
   acu.startConnection();
   var {
      lastName,
      firstName,
