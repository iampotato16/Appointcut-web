...
class BarberFetch {
   ...
   async getBarber(barberId) {
      ...
      console.log(
         `SELECT * FROM employee where EmployeeID = ${barberId} AND EmployeeType = "Barber" and Status = 1`
      );
   ...
   async getBarberAppointmentView(barberName, day, month, year) {
      ...
      const appointments = await this.connection.query(
         `${select} ${where} ${order};`
      );
   ...
   async computeCommission(employee, year, month) {
      ...
      const where = `where MONTH(Date) = ${
         parseInt(month) + 1
      } and YEAR(Date) = ${year} and EmployeeID = ${
         employee.EmployeeID
